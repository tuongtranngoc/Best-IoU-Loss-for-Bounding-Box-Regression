...
def compute_loss(target_bboxes, pred_bboxes):
    ...
    x1 = torch.min(target_bboxes[..., 0], pred_bboxes[..., 0])
    y1 = torch.min(target_bboxes[..., 1], pred_bboxes[..., 1])
    x2 = torch.max(target_bboxes[..., 2], pred_bboxes[..., 2])
    y2 = torch.max(target_bboxes[..., 3], pred_bboxes[..., 3])

    intersects = torch.clamp((x2-x1) * (y2-y1), min=0)
