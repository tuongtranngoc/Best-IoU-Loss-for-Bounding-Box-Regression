...
if __name__ == "__main__":
    ...
    print("Testing losses function ...")
    ds = ('dataset/VOC/images', 'dataset/VOC/labels', ['dataset/VOC/images_id/test2007.txt'], False)
    val_dataloader = DataLoader(ds, batch_size=2, shuffle=True)
    model = YoloModel(
            input_size=cfg["image_size"][0],
            backbone='resnet18',
            num_classes=cfg["C"],
            pretrained=True,).to('cpu')
    loss_fn = SumSquaredError(apply_IoU=cfg["apply_IoU"]).to('cpu')
    for images, labels in val_dataloader:
        ...
        box_loss, conf_loss, class_loss = loss_fn(labels, out)